...
from utils.Standard import final_devide
...
def vis_bbox(img, bbox, label=None, score=None, ax=None):
    ...
    label_names = list(Trash_BBOX_LABEL_NAMES) + ['bg']
    # add for index `-1`
    ...
    for i, bb in enumerate(bbox):
        ...
        if label is not None and label_names is not None:
            ...
            class_name, color_name = final_devide(label_names[lb])
        ...
        ax.add_patch(plot.Rectangle(
            xy, width, height, fill=False, edgecolor=color_name, linewidth=2))

        if len(class_name) > 0:
            # ax.text(bb[1], bb[0] + img.shape[1] * 0.04,
            ax.text(bb[1], bb[2],
                    class_name,
                    style ='italic',
                    color = color_name,
